...
module.exports = {
    ...
    return new Promise((resolve, reject) => {
      ...
      tmpProt.get({protocol: tmpURL.protocol, hostname: tmpURL.hostname, port: tmpURL.port, path: tmpURL.path, headers: theHeaders}, resp => { // A GET request using options
        if (resp.statusCode === 200) { // The normal response
          ...
          this.promGET(THE_URL.parse(resp.headers.location).hostname ? resp.headers.location : THE_URL.resolve(theURL, resp.headers.location), theHeaders)
            .then(resolve)
  ...
  promPOST: function (theURL, theData, theHeaders) {
    return new Promise((resolve, reject) => {
      ...
      tmpProt.request({method: 'POST', protocol: tmpURL.protocol, hostname: tmpURL.hostname, port: tmpURL.port, path: tmpURL.path, headers: theHeaders}, resp => { // A POST request using options
        if (resp.statusCode === 200 || resp.statusCode === 201) { // In case there was a direct content for the result of POST
          let tmpPayLoad = []
          resp.addListener('data', ch => tmpPayLoad.push(ch)).once('end', () => resolve(Buffer.concat(tmpPayLoad))) // Resolve the payload
        } else if (resp.statusCode === 204) { // In case of Created or No-Content
          ...
          // In case the redirection is to a full URL path or not, we'll resolve the location
          this.promPOST(THE_URL.parse(resp.headers.location).hostname ? resp.headers.location : THE_URL.resolve(theURL, resp.headers.location), theData, theHeaders)
            .then(resolve)
  ...
  /**
   * A DELETE HTTP method that uses promise (made basically for github api).
   * @param {String} theURL the URL to run DELETE against
   * @param {String} theHeaders the additional headers if needed
   * @returns {Promise<Buffer>} a promise that holds the response payload data if any and the last URL used
   */
  promDELETE: function (theURL, theHeaders) {
    return new Promise((resolve, reject) => {
      let tmpURL = THE_URL.parse(theURL)
      let tmpProt
      if (theURL.startsWith('https')) {
      ...
      tmpProt.request({method: 'DELETE', protocol: tmpURL.protocol, hostname: tmpURL.hostname, port: tmpURL.port, path: tmpURL.path, headers: theHeaders}, resp => { // A DELETE request using options
        if (resp.statusCode === 204) { // In case  of No-Content (the normal response from github -may add 200 here-)
          resolve('') // No content
        } else if (resp.statusCode >= 301 && resp.statusCode <= 303 && resp.headers.location) { // In case we should repeat with GET
          // In case the redirection is to a full URL path or not, we'll resolve the location
          this.promGET(THE_URL.parse(resp.headers.location).hostname ? resp.headers.location : THE_URL.resolve(theURL, resp.headers.location), theHeaders)
            .then(resolve)
            .catch(err => reject(err))
        } else if (resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location) { // In other 3xx cases with a location, then there was a redirection and we have to repeat with DELETE on redirection
          // In case the redirection is to a full URL path or not, we'll resolve the location
          this.promDELETE(THE_URL.parse(resp.headers.location).hostname ? resp.headers.location : THE_URL.resolve(theURL, resp.headers.location), theHeaders)
            .then(resolve)
            .catch(err => reject(err)) // Recursive call to the redirected URL
        } else { // Any other uninteresting response (we don't want this in our assignment)
          return reject(new Error('The server targeted in the URL responded with an uninteresting response with a status-code: ' + resp.statusCode))
        }
      }).once('errot', err => reject(err)).end()
    })
  },
